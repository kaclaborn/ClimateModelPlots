...
NonStacked.Top10Emissions.Future.Plot <-
  ...
  labs(title = "Annual GHG emissions trajectories, top 10 current emitters", 
       subtitle = "Future projected emissions of Kyoto greenhouse gases (AR4) in CO2eq,\nSSP2 baseline scenario")

NonStacked.Top10Emissions.Future.Arranged <- 
  grid.arrange(NonStacked.Top10Emissions.Future.Plot, 
               bottom = grid.text("Source: Gütschow et al (2020). <https://doi.org/10.5281/zenodo.3638137>\nNote: Units in gigagrams (Gg) CO2eq", 
                                  x = unit(77, "pt"),
                                  just = "left",
...
NonStacked.Top10Emissions.Time.Plot <-
  ggplot(GHGTop10.EU, aes(x = year, y = value/1000000)) +
  geom_line(aes(group = country, colour = country),
            size = 1.25,
            position = "identity") +
  geom_vline(aes(xintercept = 2018),
             linetype = 2) +
  annotate("text", x = 2012, y = 19, label = "2018", size = 2.5) +
  scale_x_continuous(name = "",
                     expand = c(0, 0),
                     breaks = seq(1850, 2050, by = 25)) +
  scale_y_continuous(name = "", 
                     expand = c(0, 0),
                     limits = c(0,21),
                     breaks = seq(5, 20, by = 5),
  ...
  plot.theme.top10 + legend.guide.top10 +
  labs(title = "Annual GHG emissions trajectories, top 10 current emitters", 
       subtitle = "Historic and future projected emissions of Kyoto greenhouse gases (AR4) in CO2eq,\nSSP2 baseline scenario")
...
Stacked.Top10Emissions.Future.Plot <-
  ggplot(GHGTop10.EU %>% filter(year>=2020 & year<=2050), aes(x = year, y = value/1000000)) +
  geom_area(aes(group = country, fill = country)) +
  ...
  scale_y_continuous(name = "", 
                     ...
                     labels = c("10 million Gg", "20 million Gg", "30 million Gg", "40 million Gg")) +
  plot.theme.top10 + legend.guide.top10 +
  labs(title = "Annual GHG emissions trajectories, top 10 current emitters", 
       subtitle = "Future projected emissions of Kyoto greenhouse gases (AR4) in CO2eq,\nSSP2 baseline scenario")
...
Stacked.Top10Emissions.Time.Plot <-
  ...
  geom_vline(aes(xintercept = 2018),
             linetype = 2) +
  annotate("text", x = 2012, y = 41, label = "2018", size = 2.5) +
  scale_x_continuous(name = "",
                     expand = c(0, 0),
                     breaks = seq(1850, 2050, by = 25)) +
  scale_y_continuous(name = "", 
